        ...
        <div className="grid gap-2 grid-cols-3 mx-auto max-w-5xl sm:gap-4 sm:grid-cols-4 lg:grid-cols-6">
          {days.map(({ index, isUnlocked, isFinished }) => (
            <div key={index} className={"aspect-w-1 aspect-h-1"}>
              <a
                className={`flex items-center justify-center text-white text-3xl font-bold hover:bg-blue-900 focus:bg-blue-900 shadow-inner rounded-lg transition duration-150 ease-in-out xl:text-4xl ${isUnlocked ? "bg-blue-300" : "bg-blue-200"}`}
                href={`/day/${index}`}
                tabIndex={index}
              >
                {index}
